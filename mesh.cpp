...
mesh::mesh(std::vector<vec3D>& vertices, std::vector<GLint>& indices) :
	_vertices(vertices, indices)
{}

mesh::mesh(std::vector<vec3D>& vertices, std::vector<GLint>& indices, std::vector<vec3D>& normals, std::vector<GLint>& norm_indices, uvmap* default_uv) :
	_vertices(vertices, indices, new normal_data(normals, norm_indices))
{
	_uvmap = default_uv;
}
...
void mesh::set_uvmap(uvmap* uvmap)
{
	_uvmap = uvmap;
}
